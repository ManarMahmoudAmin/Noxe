...
export default function Login({saveUserData}) {
  ...
  return (
    <>      
      ...
      {(error.length > 0) ? <div className='alert alert-danger mx-5'>{error}</div> : ''}
      <form onSubmit={submitForm} className='mx-5 py-4'>
        ...
        <button type='submit' className='btn btn-outline-info my-2'>{isLoading === true ? <i className='fas fa-spin fa-spinner'></i> : 'Login'}</button>
      </form>
    </>
